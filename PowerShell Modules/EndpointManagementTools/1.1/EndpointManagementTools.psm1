...
<#
.SYNOPSIS
    Retrieves Intune  applications based on the specified platform or retrieves all applications if the `-All` switch is used.

.DESCRIPTION
    This function uses Microsoft Graph API to fetch mobile applications managed by Intune. 
    You can filter the applications by platform (macOS, Windows, Android, iOS/iPadOS) or retrieve all apps using the `-All` parameter.

.PARAMETER Platform
    Specifies the platform for which you want to retrieve apps. Valid options are: macOS, Windows, Android, iOS.
    - macOS: Returns macOS apps (VPP, Lob, Pkg, OfficeSuite, and MicrosoftEdge).
    - Windows: Returns Windows apps (Win32, Winget, MSI, OfficeSuite, WebApp, MicrosoftEdge).
    - Android: Returns Android apps (ManagedStore, Lob, Store).
    - iOS: Returns iOS/iPadOS apps (VPP, Lob).

.PARAMETER All
    If specified, retrieves all apps across all platforms regardless of the platform filter.

.EXAMPLE
    Get-IntuneApps -Platform macOS
    Retrieves all macOS apps.

.EXAMPLE
    Get-IntuneApps -All
    Retrieves all apps across all platforms.

.NOTES
    This function requires the Microsoft Graph PowerShell SDK and permission to access Intune app data.
    If not authenticated, the function will prompt for Microsoft Graph authentication.
#>
