...
foreach ($value in $configurationPolicyValues) {
    $configid = $value.Id  # Fix: Define configid variable
    switch ($value.Platform) {
        "windows10" {
            $string = $value.Name -replace '^(?:CFG - )?(?:Windows|Win10)\s*-\s*', ''
            $displayName = "CFG - Win10 - $string"
    ...
    try {
        ...
        Write-Output "Failed to update profile: $DisplayName - Error: $($_.Exception.Message)"
    }
...
$URI = "https://graph.microsoft.com/beta/deviceManagement/configurationPolicies('d9f32c76-da78-492f-ab1f-27192361b127')"
$body = @{
    "name" = "CFG - Win10 - Add to RDP Group"
} | ConvertTo-Json -Depth 10
